...
contract NFTMarket is ITokenReceiver {
    ...
    bytes32 public constant WHITELIST_TYPEHASH = keccak256("Whitelist(address buyer,uint256 listingId,uint256 nonce,uint256 deadline)");
    ...
    // 构造函数，设置支付代币地址和项目方签名者地址
    constructor(address _paymentTokenAddress, address _projectSigner) {
        require(_paymentTokenAddress != address(0), "NFTMarket: payment token address cannot be zero");
        require(_projectSigner != address(0), "NFTMarket: project signer address cannot be zero");
    ...
    function permitBuy(
        ...
        // 检查截止时间
        require(block.timestamp <= _deadline, "NFTMarket: permit expired");
        ...
        require(recoveredSigner != address(0), "NFTMarket: invalid signature");
        require(recoveredSigner == projectSigner, "NFTMarket: unauthorized buyer");
